--- chrome/common/chrome_paths.cc.orig	2019-11-19 01:38:21 UTC
...
+          "/usr/local/etc/chrome/policies"));
 ...
 #endif
@@ -489,7 +492,7 @@ bool PathProvider(int key, base::FilePath* result) {
     }
 #endif
 #if defined(OS_CHROMEOS) || \
...
+#if defined(OS_LINUX) || defined(OS_BSD)
     case chrome::DIR_STANDALONE_EXTERNAL_EXTENSIONS: {
       ...
       break;
@@ -532,7 +535,7 @@ bool PathProvider(int key, base::FilePath* result) {
 #endif
       break;
...
+#if defined(OS_LINUX) || defined(OS_MACOSX) || defined(OS_BSD)
 ...
 #if BUILDFLAG(GOOGLE_CHROME_BRANDING)
@@ -546,6 +549,9 @@ bool PathProvider(int key, base::FilePath* result) {
 #if BUILDFLAG(GOOGLE_CHROME_BRANDING)
       cur = base::FilePath(FILE_PATH_LITERAL(
...
@@ -558,7 +564,7 @@ bool PathProvider(int key, base::FilePath* result) {
         return false;
